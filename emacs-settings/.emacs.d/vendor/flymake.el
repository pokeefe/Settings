...
(defun flymake-parse-err-lines (err-info-list lines)
  ...
  (let* ((count              (length lines))
	...
	;;   (setq err-info-list (flymake-add-err-info err-info-list line-err-info))))
	  ...
	  (setq line-err-info (flymake-ler-set-file line-err-info nil))
	  (setq err-info-list (flymake-add-err-info err-info-list line-err-info)))
      ;; (flymake-log 3 "parsed '%s', %s line-err-info, The output is  %s, real %s, source %s" (nth idx lines) (if line-err-info "got" "no") line-err-info real-file-name source-file-name)
      (setq idx (1+ idx)))
    err-info-list))
...
Returns ((LINES) RESIDUAL)."
  (when (and output (> (length output) 0))
    (let* ((lines (flymake-split-string output "[\n]+")) ; did have a r
	   ...
	   (residual nil))
      ;; (flymake-log 3 "The OUTPUT guy is %s " output)
      ;; (flymake-log 3 "The LINES guy is: %s" lines)
      (when (not complete)
...
For the format of LINE-ERR-INFO, see `flymake-ler-make-ler'."
  ...
  (let* ((line-no             (if (flymake-ler-file line-err-info) 1 (flymake-ler-line line-err-info)))
	 ...
	 (err-info            nil))
;; (flymake-log 3 "Attempting to add to the err-info-list with results %s, %s, %s" err-info-list exists line-err-info-list)
    (if exists
...
(defun flymake-safe-delete-file (file-name)
  (when (and file-name (file-exists-p file-name))
    ...
    (flymake-log 1 "Did not! deleted file %s" file-name)))
